...
def add_results_for_targets(targets, values):
    results = []
    for c, result_set in enumerate(values):
        # Handle multiple results in a single period.
        value_list = [result_set] if not isinstance(result_set, list) else result_set
        for d, value in enumerate(value_list):
            date_collected = targets[c].start_date + datetime.timedelta(days=d)
...
class TestAnnualNonSummingCumulativeNumericLarge(TestAnnualNoncumulativeNumeric):
    ...
    lop_actual_progress = 25
    lop_percent_met_progress = 1.25
    target_values = [10, 20, 50, 90]
