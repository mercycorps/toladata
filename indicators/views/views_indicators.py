...
@method_decorator(login_required, name='dispatch')
class ParticipantCountFiscalExport(APIView):
    """
    Class based API View for generating and exporting a CSV with all of the particpant count results for the fiscal year.
    ...
    def sum_counters(counters, start):
        """
        Utility method for adding multiple counter objects together.

        Params
            counters
                list of Counter objects
            start
                default counting dict

        Returns
            A single Counter object will all totals
        """
        for counter in counters:
            for key in counter:
                for nested_key in counter[key]:
                    start[key][nested_key] += counter[key][nested_key]
    ...
    def remove_new_lines(string):
        """
        Utility method for removing newlines from a string.
        If a string is imported to csv with a new line all text following the newline is removed.
        """
    ...
    def comma_separate_list(separate_list):
        """
        Utility method to separate a list into a comma separated string

        Params
            separate_list
                - List to be converted to a comma separated string
    ...
    @staticmethod
    def create_default_counting_dict(top_level_keys, secondary_keys):
        """
        Utility method for generating the default counting dict. Used for sectors and sadd counting dicts.

        An example counting dict:
        {
            "Agriculture": {
                "Sectors Direct with double counting": 0,
                "Sectors Indirect with double counting": 0
            }
        }

        Params
            top_level_keys
                The keys used to initially index the dictionary
            secondary_keys
                The keys used to index the nested dictionary

        Returns
            dictionary following the above example
        """
        counting_dict = {}

        for top_level_key in top_level_keys:
            counting_dict[top_level_key] = {
                secondary_keys[0]: 0,
                secondary_keys[1]: 0
            }
    ...
    def add_sector_sadd_data(self, counting_dict, target_keys):
        """
        Loops through the counting dict and appends each value to the csv_row

        Params
            counting_dict
                The counting_dict for either sadd or sector
            target_key
                The nested key in counting_dict
        """
        for target_key in target_keys:
            for key in counting_dict:
                self.csv_row.append(counting_dict[key][target_key])
    ...
    def get_country_region_list(self, indicator):
        ...
        for country in indicator.program.country.all():
    ...
    def get_outcome_themes(self, result):
        """
        Returns the outcome_themes for the current result
        ...
        Returns
            outcome_themes as a set
        """
        outcome_theme_set = set()

        for outcome_theme in result.outcome_themes.all():
            outcome_theme_set.add(outcome_theme.name)

        return outcome_theme_set

    def get_sector_data(self, result):
        """
        Populates counting_dict with all the sector values
        ...
        Returns
            populated counting_dict with type of Counter
        """
        # counting_dict = dict.fromkeys(self.sector_list, dict.fromkeys(self.sector_keys, 0))
        counting_dict = self.create_default_counting_dict(self.sector_list, self.sector_keys)

        counting_dict = self.get_disaggregated_values(result, counting_dict)

        return Counter(counting_dict)

    def get_sadd_data(self, result):
        """
        Populates counting_dict with all sadd values
        ...
        Returns
            populated counting_dict with type of Counter
        """
        # counting_dict = dict.fromkeys(self.sadd_list, dict.fromkeys(self.sadd_keys, 0))
        counting_dict = self.create_default_counting_dict(self.sadd_list, self.sadd_keys)
    ...
    def get_actuals(self, result):
        ...
        Returns
            populated counting_dict with type of Counter
        """
        counting_dict = self.create_default_counting_dict(self.actual_list, self.actual_keys)

        counting_dict = self.get_disaggregated_values(result, counting_dict)

        return Counter(counting_dict)

    def add_actuals(self, counting_dict):
        """
        Adds the actual values and totals to the csv_row

        Params
            counting_dict
                counting_dict for actuals
        """
        self.csv_row.extend([
            counting_dict['Direct']['Actual without double counting'], counting_dict['Indirect']['Actual without double counting'],
            sum([counting_dict['Direct']['Actual without double counting'], counting_dict['Indirect']['Actual without double counting']]),
            counting_dict['Direct']['Actual with double counting'], counting_dict['Indirect']['Actual with double counting'],
            sum([counting_dict['Direct']['Actual with double counting'], counting_dict['Indirect']['Actual with double counting']])
        ])

    def get_disaggregated_values(self, result, counting_dict):
        """
        Loops through the disaggregated values for the result object. If the result is in the counting dict then adds the value

        Params
            result
                The current results object
            counting_dict
                The counting_dict to add values to
        """
        for disagg in result.disaggregatedvalue_set.all():
            try:
                counting_dict[disagg.category.name][disagg.category.disaggregation_type.disaggregation_type] += disagg.value
            except KeyError:
                continue

        return counting_dict

    def populate_results(self, indicator):
        """
        Loops through the indicators results. Call the methods to get each value
        ...
        for key in results_dict:
            ...
            value = self.sum_counters(results_dict[key], Counter(self.create_default_counting_dict(
                getattr(self, "{}_list".format(key)), getattr(self, "{}_keys".format(key))
            )))

            if key == 'actual':
                self.add_actuals(value)
            else:
                self.add_sector_sadd_data(value, getattr(self, "{}_keys".format(key)))

    def populate_row(self, indicator, writer):
        """
        Begins populating the row with all the easier values. Then calls populate_results for the more nested values.
    ...
    def get(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = 'attachment; filename="participant_count_fiscal_{}_export_{}.csv"'.format(
            self.event_target_period, date.today().isoformat()
        )
