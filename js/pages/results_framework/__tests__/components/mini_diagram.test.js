...
describe("Tree Diagram", () => {
    ...
    describe("given a variety of container widths, node heights, and tier counts", () => {
        ...
        test.each([[320], [150], [500]])("appropriately sizes svg width", (containerWidth) => {
            const svgNode = render(<RFTreeDiagram containerWidth={containerWidth} levelTreeData={ levelData } inactiveTiers={[]} clickHandlerGetter={ clickHandlerGetter } />);
        ...
        test.each([[40], [60], [100]])("appropriately sizes svg height to 4.25 * node height", (nodeHeight) => {
            ...
            const svgNode = render(<RFTreeDiagram nodeHeight={ nodeHeight } levelTreeData={ levelData } inactiveTiers={[]} clickHandlerGetter={ clickHandlerGetter } />);
        ...
        test.each([
            ...
            const svgNode = render(<RFTreeDiagram levelTreeData={ thisLevelData } inactiveTiers={[]} clickHandlerGetter={ clickHandlerGetter }/>);
            expect(svgNode.attr('height')).toBe(expectedHeight);
        });
    });
});
